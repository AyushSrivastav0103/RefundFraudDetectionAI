...
class FraudDetectionGraph:
    ...
    def ml_check(self, state: Dict[str, Any]) -> Dict[str, Any]:
        """Run ML model + heuristic scoring."""
        import pandas as pd
        ...
        # Heuristic text similarity
        heur = self.model.predict_proba_text(
            state["claim_text"], state.get("user_history", "")
        )
    ...
    def faiss_check(self, state: Dict[str, Any]) -> Dict[str, Any]:
        """Retrieve similar claims via FAISS."""
    ...
    def llm_reasoning(self, state: Dict[str, Any]) -> Dict[str, Any]:
        """LLM structured reasoning with guardrails (JSON enforced)."""
        prompt = PromptTemplate.from_template(
            ...
            Task:
            - Decide if this claim is suspicious.
        ...
        # Guardrail: enforce JSON
        try:
            parsed = json.loads(getattr(raw_response, "content", raw_response))
            state["llm_fraud_probability"] = parsed.get("fraud_probability", 0.0)
            state["llm_reasoning"] = parsed.get("reasoning", "")
        except Exception:
            # fallback
            state["llm_fraud_probability"] = state["ml_score"]
            state["llm_reasoning"] = "Could not parse LLM response. Falling back to ML score."
        return state
    ...
    def final_decision(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "llm_fraud_probability": state.get("llm_fraud_probability", 0.0),
            "llm_reasoning": state.get("llm_reasoning", "(no rationale)"),
        }
