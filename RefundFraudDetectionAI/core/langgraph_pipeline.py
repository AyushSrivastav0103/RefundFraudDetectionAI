...
class FraudDetectionGraph:
    ...
    def ml_check(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Heuristic score from text/history
        ...
        state["ml_score"] = state["ml_score_blended_pct"]  # backward compat
        return state
    ...
    def llm_reasoning(self, state: Dict[str, Any]) -> Dict[str, Any]:
        prompt = PromptTemplate.from_template(
            """
            You are an AI fraud investigator analyzing a customer refund claim.

            Inputs:
            - Claim: {claim}
            - Past History: {history}
            - ML Fraud Probability (0–100): {ml_score}
            - Similar Past Fraudulent Claims: {similar}

            Task:
            - Assess whether this claim is suspicious.
            - Respond ONLY in valid JSON with exactly two fields:
              {{
                "fraud_probability": number (0-100),
                "reasoning": "Detailed but concise explanation (5-7 sentences) of why this claim is suspicious or not."
              }}
            - Do NOT include extra text, markdown, or commentary outside JSON.
            """
        )
        ...
        parsed = {"fraud_probability": None, "reasoning": raw_output}

        # Try parsing JSON safely
        try:
            parsed_json = json.loads(raw_output)
            if isinstance(parsed_json, dict):
                parsed["fraud_probability"] = parsed_json.get("fraud_probability")
                parsed["reasoning"] = parsed_json.get("reasoning", raw_output)
        except Exception:
            # Fallback: keep raw text in reasoning
            pass

        state["llm_reasoning"] = parsed["reasoning"]
        state["llm_score"] = parsed["fraud_probability"]
        return state
    ...
    def final_decision(self, state: Dict[str, Any]) -> Dict[str, Any]:
        return {
            ...
            "llm_score": state.get("llm_score", None),  # parsed JSON probability
        }
